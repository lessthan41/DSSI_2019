...
# url_reHospital 網址存取失敗(網頁顯示系統發生錯誤)，因此略過
#res1 <- GET(url_reHospital) %>%
#    content("text") %>%
#    fromJSON()
...
res <- GET(url_104) %>% content("text") %>% fromJSON()
class(res)
...
safefood.m <- matrix(safefood.v, byrow = TRUE, ncol = 5)
safefood.m %>% head(20)
...
safefood.df <- safefood.df %>% select(-4)
