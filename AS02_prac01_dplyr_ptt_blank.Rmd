...
comments %>%
  ...
  # summarise
  select(commentor, order) %>%
  summarise(avg_order = mean(order),
            sd_order = sd(order),
            tot_comment = n()) %>%
  
  # find avg_order top 10
  arrange(avg_order) %>%
  slice(1:10)
...
comments %>%
  ...
  group_by(commentor, plink) %>% # 根據使用者、其回應文章分類
  summarise(num_this_post = n()) %>% # 該使用者於該文章留言幾則
  ungroup() %>%
  group_by(commentor) %>%
  
  summarise(num_post = n_distinct(plink),
            max_c2p = max(num_this_post),
            min_c2p = min(num_this_post),
            mean_c2p = mean(num_this_post),
            sd_c2p = sd(num_this_post)) %>%
